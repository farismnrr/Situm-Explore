...
export function useExploreIndoorRoute(input: {
  ...
  function requestIsCurrent(requestId: number, workspaceId: string, buildingId: number, fromPoiId: number, toPoiId: number) {
    return requestGeneration === requestId
      && input.workspaceId.value === workspaceId
      && input.buildingId.value === buildingId
      && routeStartPoiId.value === fromPoiId
      && routeDestinationPoiId.value === toPoiId
  }
  ...
  async function calculateRoute() {
    ...
    try {
      ...
      if (!requestIsCurrent(requestId, workspaceId, buildingId, from.id, to.id)) return
      ...
      if (!result.ok) {
        ...
        routeError.value = result.reason === 'no-path-data'
          ...
          : result.reason === 'unsupported-route-tags'
            ? 'This venue uses Situm route tags that static web routing does not support yet.'
