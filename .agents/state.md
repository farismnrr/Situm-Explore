...
Physical-device acceptance exposed a client bug in the already-shipped updater: tapping `Download update` could appear to do nothing because the app owned APK download/installer handoff and swallowed failures. The remediation follows the proven Sensio Notes pattern instead: the app opens the immutable release URL in the device browser, the backend signs the private-S3 APK with `Content-Disposition: attachment`, and Android/browser download handling owns the transfer; users install the downloaded APK from Downloads. Direct APK cache/download code, installer intents, `expo-device`, `expo-intent-launcher`, the direct `expo-file-system` dependency, and `REQUEST_INSTALL_PACKAGES` are no longer part of this flow. Physical POS acceptance passed on 2026-09-14 with a test-only `0.1.0` / versionCode `4` build: the update modal appeared without login, Chrome opened the versioned `0.1.1` route, the APK downloaded successfully, and the device-side SHA-256 matched production `5aa468491bc941c0c1b5f1e68c4192c807c930c87a0234109647ba3a11eb98ce`. The POS was then restored to the immutable production `0.1.1` / versionCode `5` build. Shipping the remediation must use a new Android release (`0.1.2` / versionCode `6` or later); production `0.1.1` must never be overwritten.
