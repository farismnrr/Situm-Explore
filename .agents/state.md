...
Plan 044 — Web / Native Map UI Parity is **closure-complete** on `plan/044-web-native-map-parity` at closure commit `b39c25c6e7ae6242ab4b18b4f4e7653fae898052`, pushed to origin and awaiting user-gated PR/integration. The user explicitly superseded the interim 2D web Map direction on 2026-09-10: `/app/map` is a **3D-only** app-owned Three.js/WebGL digital-twin walkthrough with an eye-level 1.62 m perspective camera. There is no 2D floorplan/Viewer fallback; missing GLB, WebGL, or discoverable semantic-room data is an explicit error. The browser owns mouse-look, WASD/arrow/touch walking controls, floor switching, room search, reset/fullscreen, and deterministic camera travel to a selected room. Current Situm cartography returns zero POIs, so browser destinations are canonical semantic room objects extracted from the trusted floor GLB rather than fabricated POIs. Floor-scoped GLBs are served through authenticated workspace scope and mounted read-only in staging; no Situm credential is issued to the renderer. Sensor-backed blue-dot positioning and turn-by-turn guidance remain native-only. Authenticated Playwright acceptance passed for F2 (`Workroom 3`, camera X 8.450→3.325) and F1 (`Workroom 1`, camera X 8.401→5.325), with GLB responses `200 model/gltf-binary`, valid `glTF` magic, active WebGL rendering, and eye height preserved at 1.620 m. User visual E2E is not claimed as executed; the user explicitly requested governance closure before the next stage. Closure cleanup removed the superseded 2D renderer and duplicate 3D route, split semantic-room extraction out of the 3D renderer, kept `server/utils` at its existing maintainability ceiling, and refreshed `@nuxt/ui`/transitive resolutions so clean `npm ci` reports 0 vulnerabilities. Final clean-branch `Engineering Guard web full` passed policy, lint, typecheck, production build, and audit; `maintainability.py web` passed with no new file/function/bypass debt. The first guard attempt stopped at policy because `.tmp-tests` remained; those task-owned temporary artifacts were inspected and deleted before the successful rerun. The user's pre-existing Drizzle/schema edits for PostgreSQL `public` were restored after closure and remain unstaged/uncommitted. Final local staging was rebuilt as `sha-b39c25c6e7ae-publicschema-local`, intentionally combining the closure source with that required DB compatibility worktree; only the web service was force-recreated, it is healthy, and `/api/health/liveness` plus `/` return 200. No browser automation ran during the final rebuild. GHCR publication remains blocked by local 403 push authorization.
...
Latest **integrated** product work on `main` is Plan 041 through PR #39 at merge commit `99318a608af24f036e1744d9e7e929b859fbe15e` (`feat(mobile): ship app-owned indoor navigation`), following PR #38 at `845b437` for explicit Explore fullscreen. The previously separate Android 1.0.2 OTA history is now preserved on `main` in `.agents/evidence/android-release-1.0.2-2026-08-28.md` and `.agents/sessions/2026-08-28.md`; the old release branch is no longer needed as a history carrier.
...
Continue future work from a fresh dedicated plan branch based on updated `main`. Historical plans, sessions, evidence, and retired branches are context only unless explicitly re-opened.
