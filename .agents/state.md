...
Plan 046 — Web Explore 2D Routing Foundation is **active; Phase 0 is complete and product implementation is underway on `plan/046-web-explore-2d-routing-foundation`**. The accepted Phase 0 blocker was resolved with a temporary local-only authenticated Viewer harness and fresh A→B/B→A reference screenshots for real POIs `1274449` (PT Perkasa Pilar Utama) and `1274451` (BCA Cab Artha Gading Niaga), both on floor `69907` / Base in building `19870`. The selected graph contains 1 path object, 22 nodes, and 29 links; every link is `origin: both`, accessible, and untagged, so no material tag/profile ambiguity remains for the mandatory same-floor parity proof. The Viewer remains acceptance-only and is not part of `/app/map`. Current execution authority remains `.agents/execution/plan-046-web-explore-2d-routing-foundation.md`; Plan 046 still must preserve the app-owned 2D + Three.js 3D renderers, use one renderer-independent `IndoorRoute`, prove custom A→B/B→A topology against the fresh Situm oracle, omit 3D route rendering, and stop at the Leader Review Gate.
