...
Plan 044 — Web / Native Map UI Parity is active on `plan/044-web-native-map-parity`. The user explicitly authorized replacing the web Map's primary embedded Viewer presentation with an app-owned responsive renderer aligned to the native Explore UI. The current implementation renders real workspace floorplans/POIs, search/floor context, collision-aware labels, destination UI, pan/pinch/wheel zoom, reset/fullscreen, and native-app handoff without issuing a Situm credential to the browser Map. Sensor-backed blue-dot positioning and turn-by-turn guidance remain native-only. A user screenshot exposed that the first deployed build rendered only the canvas because `/app/map` referenced nonexistent `MapMapSearchDock`, `MapMapControlStack`, and `MapMapDestinationCard` auto-import names; these are now corrected to `MapSearchDock`, `MapControlStack`, and `MapDestinationCard`, and the web navigation/page label is `Explore` to match the React Native destination. Focused ESLint, Nuxt typecheck, `git diff --check`, and production build pass; the build now includes all Explore overlay chunks. Local production-style deployment on port 3005 is healthy using image `sha-a3f4bf1a1665-explorefix-local`, which combines the Plan 044 Explore fix with the user's pre-existing uncommitted public-schema Drizzle compatibility edits. The actual `situm_explore` database stores application tables in `public`, while committed `main` still targets a `situm_explore` PostgreSQL schema; dummy and known-user wrong-password login smokes return 401 with no query/runtime error. GHCR publication remains blocked by a local 403 push authorization failure, so the currently healthy image is local-only. Authenticated browser visual re-check after the Explore overlay fix is still pending user confirmation. The user's Drizzle/schema edits remain unstaged and uncommitted.
