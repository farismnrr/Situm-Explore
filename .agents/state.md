...
Plan 044 — Web / Native Map UI Parity is active on `plan/044-web-native-map-parity`. The user explicitly authorized replacing the web Map's primary embedded Viewer presentation with an app-owned responsive renderer aligned to the native Explore UI. The current implementation renders real workspace floorplans/POIs, search/floor context, collision-aware labels, destination UI, pan/pinch/wheel zoom, reset/fullscreen, and native-app handoff without issuing a Situm credential to the browser Map. Sensor-backed blue-dot positioning and turn-by-turn guidance remain native-only. Focused ESLint, Nuxt typecheck, and a production build pass. Local production-style deployment on port 3005 is healthy using image `sha-a3f4bf1a1665-publicschema-local`, which combines Plan 044 with the user's pre-existing uncommitted public-schema Drizzle compatibility edits. This was required because the actual `situm_explore` database stores application tables in `public`, while committed `main` still targets a `situm_explore` PostgreSQL schema; the clean Plan 044 image therefore produced login 500s until the compatibility build was deployed. Dummy and known-user wrong-password login smokes now return 401 with no query/runtime error. GHCR publication remains blocked by a local 403 push authorization failure, so the currently healthy image is local-only. Authenticated browser Map visual acceptance is still not claimed. The user's Drizzle/schema edits remain unstaged and uncommitted.
