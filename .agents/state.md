...
Plan 044 — Web / Native Map UI Parity is active on `plan/044-web-native-map-parity`. The user explicitly superseded the interim 2D web Map direction on 2026-09-10: `/app/map` is now a **3D-only** app-owned Three.js/WebGL digital-twin walkthrough with an eye-level 1.62 m perspective camera. There is no 2D floorplan/Viewer fallback; missing GLB, WebGL, or discoverable semantic-room data is an explicit error. The browser owns mouse-look, WASD/arrow/touch walking controls, floor switching, room search, reset/fullscreen, and deterministic camera travel to a selected room. Current Situm cartography returns zero POIs, so browser destinations are canonical semantic room objects extracted from the trusted floor GLB rather than fabricated POIs. Floor-scoped GLBs are served through authenticated workspace scope and mounted read-only in staging; no Situm credential is issued to the renderer. Sensor-backed blue-dot positioning and turn-by-turn guidance remain native-only. Focused ESLint/Nuxt typecheck and production Docker build pass. Authenticated Playwright acceptance using the user-provided owner account passed for both F2 (`Workroom 3`, camera X 8.450→3.325) and F1 (`Workroom 1`, camera X 8.401→5.325), with GLB responses `200 model/gltf-binary`, valid `glTF` magic, active WebGL renderer, and eye height preserved at 1.620 m. Temporary E2E scripts were deleted and Chrome/Chromium/Playwright process count was zero afterward. User visual E2E is intentionally the remaining acceptance step. Staging is currently healthy on local-only image `sha-523affb4c049-e2e-local`, rebuilt after commit `523affb4c049d17630fe94d73956572b142be686` and force-recreated on port 3005; liveness returns 200. The image intentionally includes the user's still-uncommitted `public`-schema DB compatibility worktree because the staging database requires it. GHCR publication remains blocked by local 403 push authorization. The user's pre-existing Drizzle/schema edits that target the database's actual `public` schema remain unstaged and uncommitted.
